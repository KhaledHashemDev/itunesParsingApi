...
private const val TAG = "ClassicViewModel"
...
class ClassicViewModel(
    ...
    private val _classicSongs: MutableLiveData<UIState> = MutableLiveData(UIState.LOADING)
    ...
    val classicSongs: LiveData<UIState> get() = _classicSongs
...
// here you can perform some operation once the ViewModel gets created
    ...
    fun getMusic(genre: Genre) {
        viewModelScope.launch(Dispatchers.IO) {
            repository2.getMusic(genre).collect {
...
// This will keep only the last value emitted, and will be slower than the set value
                _classicSongs.postValue(it)  //us this if not sure (most updated data)
            }
        }
    }
