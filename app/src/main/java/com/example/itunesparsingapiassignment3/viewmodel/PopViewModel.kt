...
private const val TAG = "PopViewModel"
...
class PopViewModel(
    ...
    private val _popSongs: MutableLiveData<UIState> = MutableLiveData(UIState.LOADING)
    ...
    val popSongs: LiveData<UIState> get() = _popSongs
...
// here you can perform some operation once the ViewModel gets created
    ...
    fun getMusic(genre: Genre) {
        viewModelScope.launch(Dispatchers.IO) {
            repository3.getMusic(genre).collect {
...
// This will keep only the last value emitted, and will be slower than the set value
                _popSongs.postValue(it)  //us this if not sure (most updated data)
            }
        }
    }
